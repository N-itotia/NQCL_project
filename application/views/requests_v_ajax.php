...
<div id = "nav_left_container">       
    <legend>
        ...
        <input type="text" class="filter_date" id="Sitart" placeholder="Start date"/>
        <input type="text" class="filter_date" id="Yend" placeholder="End date"/>
          <select id="R1" name="r1">
             <option value="">Default(No r1)</option>	
             <option value="r1">With r1</option>	
       </select>
        <button type="button" id="gen_req_dataRRRR">Generate Sample List</button>
<span style="display:none;" id="LLLOADER"><img  src="ajax-loader_1.gif"/> Please Wait......</span>
	    ...
	    <select id="R222" name="activity">
		   <option value="">-ALL-</option>
		   <?php foreach($tracking as $t){?>
		    <option value="<?php echo $t->activity;?>"><?php echo $t->activity;?></option>
		   <?php } ?>
             
       </select>
        <button type="button" id="gen_req_dataRRRR2">Generate COA List</button>
		<span style="display:none;" id="LLLOADER2"><img  src="ajax-loader_1.gif"/> Please Wait......</span>
</div>
...
//Get all the information for a single analyis request entry
        ...
        $('.upd_presentation').live("click", function (e) {
         ...
         $('#gen_req_dataRRRR').click(function () {
            ...
            $("#gen_req_dataRRRR").prop('disabled', true);
            $('#LLLOADER').show();
            $(this).prop('text', 'Generating Please Wait');
          
            start = $('#Sitart').val();
            end = $('#Yend').val();
            $.post("<?php echo base_url()?>Sample_request_details/generate/" + start + "/" + end+"/"+$('#R1').val(), function (data) {
                $(this).prop('value', 'Generate Sample List');
                $("#gen_req_dataRRRR").prop('disabled', false);
                $('#LLLOADER').hide();
		...
		$('#gen_req_dataRRRR2').click(function () {
            ...
            $("#gen_req_dataRRRR2").prop('disabled', true);
            $('#LLLOADER2').show();
            $(this).prop('text', 'Generating Please Wait');
          

            $.post("<?php echo base_url()?>Sample_request_details/generate_two/" ,{year:$('#R2').val(),activity:$('#R222').val()}, function (data) {
                $(this).prop('value', 'Generate COA List');
                $("#gen_req_dataRRRR2").prop('disabled', false);
                $('#LLLOADER2').hide();

                window.location.href = "<?php echo base_url() . 'sample_report/Report2.xlsx'; ?>";
