...
class Supervisors extends MY_Controller {
	...
	function remove(){
		...
		foreach($ids as $i):
		$this->db->where('labref',$i)->delete('tests_done');
		endforeach;
	...
	function receive($labref){
		...
		$this->db->where('labref',$labref)->update('tests_done',array('rec_status'=>1));
		$this->Received_by_Supervisor($labref);
		redirect('supervisors');
		}
	}
	
	function receiveAndApprove($labref){
		if($this->session->userdata('user_id')===''){
			redirect('user_management/login');
		}else{
		$this->db->where('labref',$labref)->update('tests_done',array('rec_status'=>1));
		$this->Received_by_Supervisor($labref);
		$this->approveSample($labref);
		}
		//redirect('supervisors');
	}
