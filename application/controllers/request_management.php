...
class Request_Management extends MY_Controller {
	...
	function showtime($labref){
		$q= $this->db->where('request_id',$labref)->get('request')->result();

		echo Carbon::createFromTimeStamp(strtotime($q[0]->designation_date))->diffForHumans(null,true);
	...
	public function client_samples() {
	 ...
	 public function listingClients() {
	   ...
	   public function requests_list_current_clients() {
        ...
        $request = $this->getAllHydratedClients();
    ...
    public function getAllHydratedClients() {
        $date = date('Y');
        $query = "SELECT r.id iid , r.request_id request_iid, r.client_id AS client_id, r.sample_qty AS sample_qty, r.sample_quantity_bup AS sample_quantity_bup, r.product_name AS product_name, r.label_claim AS label_claim, r.active_ing AS active_ing, r.dosage_form AS dosage_form, r.manufacturer_name AS manufacturer_iname, r.manufacturer_add AS manufacturer_add, r.batch_no AS batch_ino, r.exp_date AS exp_date, r.manufacture_date AS manufacture_idate, r.designator_name AS designator_name, r.designation AS designation, r.designation_date AS designation_date, r.designation_date_1 AS designation_date_1, r.urgency AS urgency, r.edit_notes AS edit_notes, r.presentation AS presentation, r.description AS description, r.product_lic_no AS product_lic_no, r.country_of_origin AS country_of_origin, r.dateformat AS dateformat, r.clientsampleref AS clientsampleref, r.moa AS moa, r.crs AS crs, r.dsgntr AS dsgntr, r.dsgntn AS dsgntn, r.packaging AS packaging, r.full_details_status AS full_details_status, r.reassigned_status AS reassigned_status, r.assign_status AS assign_status, r.oos_status AS oos_status, r.all_status AS all_status, r.rejected_status AS rejected_status, r.payment_status AS payment_status, r.split_status AS split_status, r.label_status AS label_status, r.quotation_status AS quotation_status, r.component_status AS component_status, r.coa_collection_status AS coa_collection_status, r.coa_done_status AS coa_done_status, r.invoice_print_status AS invoice_print_status, r.invoice_status AS invoice_status, r.proforma_no_status AS proforma_no_status, r.proforma_no AS proforma_no, r.client_agent_id AS client_agent_id, r.compliance AS compliance, r.priority AS priority, r.oos AS oos, r.t AS t, r.c AS c, r.user_id AS user_id, r.reject_reason AS reject_reason, r.r_count AS r_count, r.can AS can, r.quotation AS quotation, r.created_at AS created_at, r.updated_at AS updated_at, c.id AS id, c.clientid AS clientid, c.name AS client, c.address AS address, c.client_type AS client_type, c.contact_person AS contact_person, c.contact_phone AS contact_phone, c.version_id AS version_id, c.alias AS alias, c.comment AS comment, c.client_status AS client_status, c.edit_status AS edit_status, c.credit AS credit, c.email AS email, c.discount_percentage AS discount_percentage, c.discount_status AS discount_status, c.client_agent_id AS client_agent_id, c.created_at AS created_at, c.updated_at AS updated_at, p.id AS id, p.name AS name, s.id AS id, s.request_id AS request_id, s.dept AS dept, s.status AS status FROM request r LEFT JOIN clients c ON r.user_id = c.clientid LEFT JOIN packaging p ON r.packaging = p.id LEFT JOIN split s ON r.request_id = s.request_id WHERE YEAR(r.designation_date_1) = '2018' AND r.t = '1'  ORDER BY r.id DESC";
...
AND r.request_id='$i';")->result();
    ...
    public function setPresentationDescription() {
        ...
        foreach ($all_tests as $test) {
            ...
            $ib->test_id = $test['test_id'];
            if (!empty($test_charges)) {
                $ib->test_charge = $test_charges[0]['Charge'];
            }
            if ($test['test_id'] == 6) {
                $ib->method_charge = 2000;
                $ib->method_id = 31;
            }
            $ib->save();
        }
...
GROUP BY r.client_id ORDER BY popular DESC")->result();
    ...
    public function listing() {
        ...
        $data['settings_view'] = "requests_v_ajax";
        $data['info'] = Request::getAll();
		 $data['tracking'] = $this->db->get('vw_activities')->result();;
        $data['tests'] = $this->get_tests();
        $data['request'] = $this->getsamples();
        $this->base_params($data);
    }
...
//end listing
    ...
    public function add() {
		//Get Client Types
		$data['client_types'] = Client_type::getAll();
		
		//Get range of letters from A to Z
...
echo $total_time;*/
       ...
       // $data['tests_issued'] = Sample_issuance::getIssuedTests2($reqid);
        ...
        //$data['info'] = Request::getAll();
		$data['clients2'] = $this->getClients2();
		//print_r($data['info']);
        $this->base_params($data);
    }
...
$client_id = $this->input->post("client_id_1");
        $this->db->where('request_id',$reqid)->update('request',array('client_id'=>$client_id));
    }
    ...
    public function save() {
        ...
        if (!empty($cid)) {
			...
			/*Return error message on console if cid is empty -- untested*/
			echo json_encode(
				array(
					'status' => 'error',
					'message' =>  'Client does not exist. Please add them at Client Management.'
				)
			);
			
			//Incrementing client id - not good
			/* 
	...
	public function markSelectedForReanalysis(){
        ...
        for ($i = 0; $i <= count($all_uri_segments); $i++) {
            if ($i > 2) {
				...
				//Initially restriction existed on number of times sample can be re-analysed , this has since been removed.
    ...
    public function update() {
	   ...
	   public function edit_clients() {
        ...
        $data['client'] = Clients::getClientNew($data['request'][0]['user_id']);
        $data['cperson'] = $this->getClientNewC($data['request'][0]['requester']);
	...
	function getClientNewC($email) {
        $query = "SELECT name, phone FROM nqcl_clients where email='$email'";
        $person = $this->db->query($query)->result();
        return $person;
    }
