...
class Main_dashboard extends MY_Dashboard {
	...
	error_reporting(0);
        ...
        $data['diff']=  $this->date_fiff($end[0]->designation_date, $start[0]->date_added  );
    ...
    function  loadcurrdate($labref){
      return $this->db
              ->query("SELECT date_added FROM tracking_table WHERE labref='$labref' ORDER BY id DESC LIMIT 1")->result();
	...
	$sql = "select c.name,c.id as client_id, count(distinct(r.request_id)) as all_samples  
        ...
        $data['recdate'] = $end = $this->recdate($labref);
        $data['curdate'] = $start =$this->loadcurrdate($labref);
        $data['diff']=  $this->showtime($labref);
        $data['title'] = $labref. 'Tracking Details';
			$this -> load -> view('dashboard_views/tracking_report_v_1', $data);		
		}
			function showtime($labref){
				$pre = $this->db->query("SELECT * FROM tracking_table WHERE labref ='$labref' ORDER BY id DESC LIMIT 1")->result();
				//$start =$this->loadcurrdate($labref);
				if( $pre[0]->activity !='CAN No.'){
					//echo $this->switchDates($pre[0]->date_added);
			  return Carbon::createFromTimeStamp(strtotime($this->switchDates($pre[0]->date_added)))->diffForHumans(null,true);

				}else{
                $aft=$this->db->query("SELECT date_added FROM tracking_table WHERE labref='$labref' ORDER BY id DESC LIMIT 1,1")->result();	
				//echo $this->switchDates($aft[0]->date_added);
			return Carbon::createFromTimeStamp(strtotime($this->switchDates($aft[0]->date_added)))->diffForHumans(null,true);
	  ...
	  function  recdate2($labref){
      return $this->db
              ->query("SELECT designation_date "
                      . " FROM `request`"
                      . " WHERE `request_id` = '$labref'")            
              ->result();  
    }
	...
	function switchDates($date){
		$ndate = explode('-',$date);
		return $ndate[2].'-'.$ndate[1].'-'.$ndate[0].' '.date('H:i:s') ;
	}
