...
class Analyst_Controller extends MY_Controller {
	...
	function getPendingCount(){
		...
		return $this->db->query("SELECT id FROM `sample_issuance` WHERE analyst_id='$id' AND completion='0' GROUP BY lab_ref_no")->result();
	}
    ...
    public function index() {
        ...
        if ($this->checkIfHasASupervisor() === '1') {
			...
			$data['pending']=$this->getPendingCount();
    ...
    function singlesheet_stamp($labref, $sheet_name) {
        ...
        do {
            ...
            $pdf->SetXY(30, 265);
            $pdf->Write(1, $labref . ' / ' . ucfirst(str_replace('_', ' ', $sheet_name)) . ' / Download ' . $download . '  /  Analyst - ' . $full_names . ' /  Date: ' . $this->foreach(date('d-m-Y')));
            $pdf->SetXY(160, 265);
            $pdf->Write(1, 'Page ' . $pdf->PageNo() . ' of {nb}');
            $pdf->SetXY(10, 268);
    ...
    function stamp_addendum($labref) {
        ...
        do {
            ...
            $pdf->SetXY(30, 265);
            $pdf->Write(1, $labref . ' / ' . ucfirst(str_replace('_', ' ', $s_name)) . '  /  Analyst - ' . $full_names . ' /  Date: ' .$this->foreach(date('d-m-Y')));
            $pdf->SetXY(160, 265);
            $pdf->Write(1, 'Page ' . $pdf->PageNo() . ' of {nb}');
...
//        $download = $pre;
        ...
        do {
            ...
            if ($j == 0) {
                $pdf->SetXY(80, 69);
                $pdf->Write(1, $this->formdate($sample_info[0]->designation_date));
                $pdf->SetXY(158, 69);
            ...
            $pdf->SetXY(30, 265);
            $pdf->Write(1, $labref . ' /  Analyst - ' . $full_names . ' /  Date ' . $this->formdate($date));
            $pdf->SetXY(160, 265);
            $pdf->Write(1, 'Page ' . $pdf->PageNo() . ' of {nb}');
...
//        $download = $pre;
        ...
        do {
            ...
            if ($j == 0) {
                ...
                $pdf->SetXY(83, 86);                
				$pdf->MultiCell(118, 3, $sample_info[0]->active_ing, 0, 'L');

                $pdf->SetXY(83, 96);
				$pdf->MultiCell(118, 3, $sample_info[0]->description, 0, 'L');

                //$pdf->Write(1, $sample_info[0]->description);

                $pdf->SetXY(83, 115);
								$pdf->MultiCell(118, 3, $sample_info[0]->presentation, 0, 'L');
            ...
            $pdf->SetXY(30, 265);
            $pdf->Write(1, $labref . ' /  Analyst - ' . $full_names . ' /  Date: ' . $this->formdate(date('d-m-Y')));
            $pdf->SetXY(160, 265);
            $pdf->Write(1, 'Page ' . $pdf->PageNo() . ' of {nb}');
    ...
    function stamp($labref, $sheet_name) {
        ...
        do {
            ...
            $pdf->SetXY(30, 265);
            $pdf->Write(1, $labref . ' / ' . ucfirst(str_replace('_', ' ', $sheet_name)) . ' / Download ' . $download . '  /  Analyst - ' . $full_names . ' /  Date: ' . $this->formdate(date('d-m-Y')));
            $pdf->SetXY(160, 265);
            $pdf->Write(1, 'Page ' . $pdf->PageNo() . ' of {nb}');
