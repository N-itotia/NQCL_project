...
class Wordexe extends CI_Controller {
    ...
    $year = (date('Y')-1)."-".date('y');
        ...
        $section_styling->setPageSizeW(\PhpOffice\PhpWord\Shared\Converter::inchToTwip(8.268));
        $section_styling->setPageSizeH(\PhpOffice\PhpWord\Shared\Converter::inchToTwip(11.693));
        $section_styling->setMarginLeft(\PhpOffice\PhpWord\Shared\Converter::inchToTwip(1.189));
        $section_styling->setMarginRight(\PhpOffice\PhpWord\Shared\Converter::inchToTwip(1.268));
        $section_styling->setMarginTop(\PhpOffice\PhpWord\Shared\Converter::inchToTwip(1.5748));
        ...
        $styleTop1 = array('bold' => true, 'size' => 12, 'align' => 'center');
        $styleTop21 = array('size' => 12, 'underline' => \PhpOffice\PhpWord\Style\Font::UNDERLINE_SINGLE);
        $certificate_no->addText(htmlspecialchars('CERTIFICATE No.: ', ENT_COMPAT, 'UTF-8'), $styleTop1, array('space' => array('before' => 3, 'after' => 3)));
		if(@$information[0]->CAN ==="-"){
			$dataYear="CAN/".$year."/000";
		}else{
			$dataYear=@$information[0]->CAN;
		}
        $certificate_no->addText(htmlspecialchars($dataYear, ENT_COMPAT, 'UTF-8'), $styleTop21, array('space' => array('before' => 3, 'after' => 3)));
        $certificate_no->addTextBreak(1);
        ...
        $style_top_mid = array('bold' => true, 'size' => 10, 'alignment' => 'left', 'valigment' => 'middle');
        $style = array('bold' => true, 'size' => $fcfont);
        $style1 = array('size' =>$rtfont );
        //header row
        $table->addRow(400);
        $table->addCell(2200, array('bgColor' => 'D9D9D9', 'valign' => 'center', 'align' => 'center'))->addText(htmlspecialchars('PRODUCT', ENT_COMPAT, 'UTF-8'), $style, array('alignment' => 'center'));
        $table->addCell(9600, array('bgColor' => 'D9D9D9', 'gridspan' => 2,'valign' => 'center', 'align' => 'center'))->addText(htmlspecialchars($information[0]->product_name, ENT_COMPAT, 'UTF-8'), $cellVCentered);
        $table->addCell(3500, array('bgColor' => 'D9D9D9', 'valign' => 'center', 'alignment' => 'right'))->addText(htmlspecialchars('REF. No.: ' . $information[0]->request_id, ENT_COMPAT, 'UTF-8'), array('bold' => true, 'size' => 10));
        //Row 1
        ...
        $run1->addTextBreak(1);
        $date_created = date_create($information[0]->designation_date_1);
        $formated = date_format($date_created, 'd.M.Y');
        $run1->addText(htmlspecialchars($formated, ENT_COMPAT, 'UTF-8'), array('size' => $fmfont));
        ...
        $cell2->addText(htmlspecialchars($information[0]->presentation, ENT_COMPAT, 'UTF-8'), array('size' => 9));
        ...
        $run3->addTextBreak(1);
        $run3->addText(htmlspecialchars($information[0]->manufacture_date, ENT_COMPAT, 'UTF-8'), array('size' => 9));
        $table->addCell(2000)->addText(htmlspecialchars('MANUFACTURER:  ', ENT_COMPAT, 'UTF-8'), $style_top_mid);
        $cell3 = $table->addCell(5000, $gridspan);
        $cell3->addText(htmlspecialchars($information[0]->manufacturer_name, ENT_COMPAT, 'UTF-8'), array('size' => 9));
        ...
        $cell4->addText(htmlspecialchars($information[0]->manufacturer_add, ENT_COMPAT, 'UTF-8'), array('size' => 9));
        ...
        $cell5->addText(htmlspecialchars($information[0]->name . " " . $information[0]->address, ENT_COMPAT, 'UTF-8'), array('size' => 9));
        ...
        $run6->addText(wordwrap(htmlspecialchars($information[0]->clientsampleref),10,"\n",true), array('size' => $fmfont));
        ...
        $table3->addRow(200, $fancyTableCellStyle);
        ...
        foreach ($signatories as $signatures):
            ...
            $table4->addCell(2000, $new_stlye)->addText(htmlspecialchars('_____________'));
