...
class Upload extends MY_Controller {
    ...
    function coa_reviewer_uploads(){
	 ...
	 function coa_return(){
        ...
        $data['labref'] = $this->uri->segment(3);
        $data['error'] = '';
        $data['settings_view'] = 'reviewer_upload_v_return';
        //$data['supervisor'] = $this->getMySupervisor();
        $this->base_params($data);
...
function do_upload() {
        ...
        if (!$this->upload->do_upload('worksheet')) {
            ...
            if($file_name!== $tobeuploaded) {
                echo 'You have uploaded the wrong file, either the name of the file does not match the NDQ number or the extension is not .xlsx, please click the browsers back button and upload the correct file to continue';
            } else {
