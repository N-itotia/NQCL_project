...
function sourceSuggestions()
    ...
    public function rgnt_issue_list(){
        ...
        //Do Query
        $sql = "SELECT CONCAT(u.fname, ' ', u.lname) as issuee_name, r.issuer_name, r.s11_number, r.quantity_issued, r.date_issued from reagents_issuance r, user u WHERE r.reagent_id = '$id' AND u.id = r.issuee_id";
    ...
    public function rgnt_issue_list_all(){
        //Do Query
        $sql = "SELECT CONCAT(u.fname, ' ', u.lname) as issuee_name, re.name as reagent_name, re.batch_no as reagent_batch, re.date_received as reagent_rdate, re.s13_voucher, re.packaging, re.manufacturer, re.reorder_level, re.quantity, r.issuer_name, r.s11_number, r.quantity_issued, r.date_issued from reagents_issuance r, user u, reagents re WHERE r.reagent_id = re.id AND u.id = r.issuee_id";
        $query = $this->db->query($sql);
        $reagents_issue =  $query -> result_array();
...
GROUP by status")->result();
    ...
    public function reagents_save(){
      ...
      $s13_voucher = $this -> input -> post("s13_voucher");
      ...
      $reagent -> s13_voucher = $s13_voucher;
    ...
    public function reagents_issue(){
      ...
      //Get input values
      $s11_number = $this -> input -> post("s11_number");
      $reagent_id = $this -> input -> post("reagent_id");
